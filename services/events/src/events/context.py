...
def require_internal_context(request) -> InternalContext:
    # Validates request-id + tenant headers
    ctx = require_context(request)

    # Validates signature for BFF->service calls
    ...
    if not user_id:
        raise HttpError(
            401,
            cast(Any, {"code": "UNAUTHENTICATED", "message": "X-User-Id is required"}),
        )
    ...
    try:
        ...
        raise HttpError(
            400,
            cast(Any, {"code": "INVALID_USER_ID", "message": "X-User-Id must be a UUID"}),
        ) from exc
