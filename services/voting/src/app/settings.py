...
def read_allowed_hosts() -> list[str]:
    ...
    if not hosts:
        ...
        raise ImproperlyConfigured(
            "ALLOWED_HOSTS must be set to a comma-separated list of allowed hosts."
        )
    if "*" in hosts and not ALLOW_INSECURE_DEFAULTS:
        raise ImproperlyConfigured(
            "ALLOWED_HOSTS cannot contain '*' unless DJANGO_ALLOW_INSECURE_DEFAULTS=1 in DEBUG mode."
        )
    return hosts


def read_origin_list(name: str, *, insecure_defaults: list[str] | None = None) -> list[str]:
    origins = read_env_list(name)
    if origins:
        return origins
    if ALLOW_INSECURE_DEFAULTS and insecure_defaults is not None:
        return insecure_defaults
    return []


SECRET_KEY = require_env("DJANGO_SECRET_KEY", insecure_default="voting-secret")
...
BFF_INTERNAL_HMAC_SECRET = require_env(
    ...
    insecure_default="voting-internal-hmac-secret",
)
...
# CORS Configuration
CORS_ALLOWED_ORIGINS = read_origin_list(
    "CORS_ALLOWED_ORIGINS",
    insecure_defaults=["http://localhost:5173", "http://localhost:8080"],
)
...
# CSRF Configuration
CSRF_TRUSTED_ORIGINS = read_origin_list(
...
VOTING_RETENTION_PUBLISHED_OUTBOX_DAYS = int(
    os.getenv("VOTING_RETENTION_PUBLISHED_OUTBOX_DAYS", "30")
)
