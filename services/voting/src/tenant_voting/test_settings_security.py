...
class VotingSettingsSecurityTests(SimpleTestCase):
    @staticmethod
    ...
    def test_requires_secret_key_in_strict_mode(self):
        with self.assertRaises(ImproperlyConfigured) as ctx:
            self._import_settings(
                {
                    "DJANGO_DEBUG": "False",
                    "ALLOWED_HOSTS": "localhost,voting",
                    "DATABASE_URL": "postgres://user:pass@db:5432/voting_db",
                }
            )
        ...
        settings_module = self._import_settings(
            {
                "DJANGO_DEBUG": "False",
                "DJANGO_SECRET_KEY": "test-secret",
                "DJANGO_ALLOWED_HOSTS": "localhost,voting",
                "DATABASE_URL": "postgres://user:pass@db:5432/voting_db",
                "BFF_INTERNAL_HMAC_SECRET": "test-hmac-secret",
            }
        )

        self.assertEqual(settings_module.ALLOWED_HOSTS, ["localhost", "voting"])
    ...
    def test_enables_https_hardening_in_strict_mode(self):
        ...
        self.assertIn(r"^health/detailed/?$", settings_module.SECURE_REDIRECT_EXEMPT)
    ...
    def test_explicit_debug_escape_hatches_enable_local_defaults(self):
        ...
        self.assertEqual(settings_module.SECRET_KEY, "voting-secret")
