...
def _resolve_vote_permissions(user: Any) -> tuple[bool, bool]:
    ...
    # Microservices Migration: We cannot check DB directly.
    # TODO: Pass this info via JWT/Header from ID service.
    has_telegram = getattr(user, "has_telegram_link", True) 
    
    if require_telegram and not has_telegram:
        return False, True
