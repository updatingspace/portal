...
- В `stack.yml` добавлен `watchtower` (обновляет образы с тегом `latest` каждые 5 минут, `--label-enable`), а сервисы фронта и бэка помечены `com.centurylinklabs.watchtower.enable=true`. На manager-ноде Docker Swarm watchtower будет подтягивать новые теги и делать rolling restart.
