...
export const activityApiMock = {
  fetchFeedV2: vi.fn(async () => createActivityFeedV2()),
  fetchUnreadCount: vi.fn(async () => ({ count: 5 })),
  markFeedAsRead: vi.fn(async () => undefined),
  fetchAccountLinks: vi.fn(async () => createAccountLinks()),
  createAccountLink: vi.fn(async (payload: AccountLinkCreatePayload) => ({
    ...createAccountLinks()[0],
    ...payload,
    id: 999,
  })),
  deleteAccountLink: vi.fn(async () => undefined),
  fetchSources: vi.fn(async () => createSources()),
  triggerSync: vi.fn(async () => ({ ok: true, rawCreated: 0, rawDeduped: 0, activityCreated: 0 })),
  fetchSyncStatus: vi.fn(async () => createSyncStatuses()),
...
export const resetAllApiMocks = () => {
  ...
  // Activity API mocks
  activityApiMock.fetchFeedV2.mockReset().mockResolvedValue(createActivityFeedV2());
  activityApiMock.fetchUnreadCount.mockReset().mockResolvedValue({ count: 5 });
  activityApiMock.markFeedAsRead.mockReset().mockResolvedValue(undefined);
  activityApiMock.fetchAccountLinks.mockReset().mockResolvedValue(createAccountLinks());
  activityApiMock.createAccountLink.mockReset().mockImplementation(async (payload: AccountLinkCreatePayload) => ({
    ...createAccountLinks()[0],
    ...payload,
    id: 999,
  }));
  activityApiMock.deleteAccountLink.mockReset().mockResolvedValue(undefined);
  activityApiMock.fetchSources.mockReset().mockResolvedValue(createSources());
  activityApiMock.triggerSync.mockReset().mockResolvedValue({ ok: true, rawCreated: 0, rawDeduped: 0, activityCreated: 0 });
  activityApiMock.fetchSyncStatus.mockReset().mockResolvedValue(createSyncStatuses());
};
