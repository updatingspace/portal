...
export async function fetchGames(search?: string): Promise<Game[]> {
  ...
  const data = await request<ApiGame[]>(`/voting/games/${query}`);
...
export async function updateGame(id: string, payload: Partial<Game>): Promise<Game> {
  ...
  const data = await request<ApiGame>(`/voting/games/${id}`, {
    method: 'PATCH',
    body,
  });
...
export async function createGame(payload: Partial<Game>): Promise<Game> {
  ...
  const data = await request<ApiGame>('/voting/games/', {
    method: 'POST',
    body,
  });
...
export async function deleteGame(id: string): Promise<void> {
  await request(`/voting/games/${id}`, { method: 'DELETE' });
}
