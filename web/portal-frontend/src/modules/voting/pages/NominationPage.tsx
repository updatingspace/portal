...
export const NominationPage: React.FC = () => {
	...
	const options = useMemo(() => nomination?.options ?? [], [nomination?.options]);
	const limitParam =
		searchParams.get('limit') ?? searchParams.get('page_size') ?? searchParams.get('pageSize');
	...
	const pageSize = useMemo(() => Math.max(displayLimit ?? DEFAULT_PAGE_SIZE, 1), [displayLimit]);
	...
	const handleVote = async (optionId: string) => {
		...
		const canVoteNowLocal = nomination.canVote ?? false;
		...
		if (!canVoteNowLocal) {
			...
			toaster.add({
				...
				content: needsTelegramLink
					...
					: 'Нажмите “Войти” в шапке, чтобы отдать голос.',
				autoHiding: 4500,
			});
			return;
		}
			...
			setNomination((prev) =>
				prev
					? {
							...
							requiresTelegramLink: response.requiresTelegramLink ?? prev.requiresTelegramLink,
			...
			<div className="page-section">
				<div className="container">
					<div className="status-block status-block-danger">
						<div className="status-title">{errorMeta?.title ?? 'Номинация не найдена'}</div>
						<p className="text-muted mb-3">{errorMeta?.description ?? 'Проверьте ссылку или вернитесь к списку.'}</p>
						<div className="d-flex gap-2">
							<Button view="outlined" onClick={loadNomination}>
								Попробовать еще раз
							</Button>
							<Link to="/nominations">← Назад к голосованиям</Link>
						</div>
					</div>
				</div>
			</div>
	...
	const votesListLink = nomination?.voting?.id
		? `/nominations?${new URLSearchParams({ voting: nomination.voting.id }).toString()}`
		: '/nominations';
			...
			<div className="container">
				<div className="row">
					<section className="col-12 col-lg-10 mx-auto">
						<NominationHeader nomination={nomination} votesListLink={votesListLink} />
						...
						<VoteCountsToggle hasVoteCounts={hasVoteCounts} showVoteCounts={showVoteCounts} onToggle={setShowVoteCounts} />
