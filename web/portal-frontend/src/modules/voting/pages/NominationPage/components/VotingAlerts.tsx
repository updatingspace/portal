...
export const VotingAlerts: React.FC<VotingAlertsProps> = ({
  ...
  <>
    ...
    {!isVotingClosed && !canVoteNow && (
      <div className="status-block status-block-warning">
        <div className="status-title">{needsTelegramLink ? 'Нужна привязка Telegram' : 'Требуется авторизация'}</div>
        <p className="text-muted mb-0">
          {needsTelegramLink ? (
            <>
              Привяжите Telegram в профиле, чтобы участвовать в голосовании. <Link to="/me">Открыть профиль</Link>
            </>
          ) : (
            'Нажмите “Войти” в шапке, чтобы проголосовать. Пока можно изучить варианты.'
          )}
        </p>
      </div>
    )}
