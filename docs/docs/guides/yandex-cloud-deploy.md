...
Invoke URL контейнера может уже заканчиваться `/`. Перед добавлением `/api/v1`
Terraform удаляет завершающий слэш, чтобы BFF и сервисы использовали канонический
путь. Двойной слэш в `//api/v1/...` не нормализуется runtime и приводит к 404;
внутреннюю HMAC-подпись необходимо вычислять для фактически отправляемого пути.
